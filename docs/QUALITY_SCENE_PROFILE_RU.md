...
Semantic selection учитывает фактически оставшиеся timestamp после recovery и quarantine. Поэтому восстановленный singleton может попасть в описания, а потерявший второй timestamp объект туда не проходит. В каталоге сохраняются оба счётчика — исходный `independent_timestamps` и фактический `native_independent_timestamps`; `recovered_from_single_timestamp` отмечает переход. Геометрическая поддержка не удостоверяет label, физические размеры или полноту маски.
