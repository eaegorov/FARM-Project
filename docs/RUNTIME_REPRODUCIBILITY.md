...
- `requirements/bridge-control.lock.txt` is a separate exact direct-package
  launcher inventory for current ShapeR host validators, including trusted
  scene-state loading through CPU Torch. Its NumPy/SciPy pins conflict with
  the control-plane closure, so the venvs must remain separate; it requires
  Python 3.11+ and is not the inference runtime.
