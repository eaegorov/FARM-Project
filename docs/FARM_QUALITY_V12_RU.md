...
Документ содержит инфраструктуру F0/F1 и актуальный **Stage54, 2026-09-07**.
Checkpoint кода — `c1e3eb4`, 1551 tests. Последние результаты и реальные сравнения:
[целые объекты](../../../output/farm_quality_report_2026-09-07/OBJECT_QUALITY_RU.md) и
[визуализации](../../../output/farm_quality_report_2026-09-07/OBJECT_QUALITY_VISUALS_RU.md).

Опция `--whole-objects` собирает встроенные части перед итоговым распределением
Gaussian. На Factory получены 10 семейств: у части объектов заметно уменьшились
пропуски, но повторные охваты одного оборудования, ошибочные названия и неполные
поверхности остаются. Оба дополнительных Qwen-протокола разрешения дублей v3/v4
отклонены из-за зависимости ответа от порядка масок. На Knaack PCA/voxel growth
дал одно новое принятое предложение без изменения показанных 24 проекций.

Stage54 сохраняет экспериментальные каталоги отдельно. Исходный общий Factory
остаётся Stage53: 138 кандидатов, 123 непустые маски, 75 описаний; Knaack — Stage47.
Число записей не означает число правильно восстановленных физических объектов.
`--whole-objects` пока начинается с подготовленного зарегистрированного RGBD;
полный путь от сырых COLMAP+PLY и его время ещё не проверены. Универсальная
точность, абсолютные физические размеры и общий production release не подтверждены.
Ручная pixel-разметка ниже описывает дополнительную независимую оценку и не блокирует разработку.
...
Актуальные эксперименты — `54_object_scope_quality/`; пользовательские отчёты приведены выше.
История V12 — `REPORT_RU.md`, итоговый пакет независимой разметки — **`02_manual_gold/packet.json`**.
...
## Сборка целых объектов и локальное расширение поверхности

В quality-профиль добавлен необязательный флаг `--whole-objects` команды
`farm quality scene-profile plan`. Он вставляет после исходного каталога две стадии:

1. `scope_families`: выбор вложенных масок по двум независимым моментам съёмки и
   проверка отношения «целый предмет — встроенная деталь» локальной Qwen.
   Модель получает полный контекст и выделения на фотографиях; названия детектора
   не используются как истина. Запрос повторяется с обратным порядком изображений.
2. `object_assembly`: повторное разрешение исходных претензий на Gaussian на уровне
   согласованных семейств. Для одного элемента берётся одна исходная строка
   свидетельств, поэтому число деталей не увеличивает число подтверждений.
   Граф направленный: общий спорный дочерний элемент не объединяет два разных
   родителя. Встроенные уровни объединяются только при единственном корневом объекте.

Результат сохраняется отдельно: `quality/object_assembly/catalog.json`
(`farm.quality-object-assembly.v1`) и `object_masks.npz`. Исходные карточки деталей
и базовый банк доступны через `source_catalog` / `source_native_masks` в artifacts.
Повторный запуск использует общую DAG-инфраструктуру:

```bash
farm run --config profile.json --run-id whole-v1
farm status --config profile.json --json
```

`profile.json` создаётся той же командой `scene-profile plan` со всеми параметрами
исходной сцены и дополнительным `--whole-objects`. Ручных списков номеров объектов
эта опция не требует. Пока это проверяемая экспериментальная возможность;
согласие VLM не является разметкой истинного состава объекта.

Отдельный `quality.normal_growth.graph_surface_candidates` создаёт ограниченные
предложения расширения по локальному графу поверхности. Учитываются нормали,
межповерхностные разрывы, занятые соседями воксели и бюджет распространения.
Функция не присваивает метки и не заполняет объём OBB. Предложения должны пройти
обычные проверки изображений, конкуренции и исходного бюджета роста.

**Граница автоматизации:** этот quality-профиль начинается с подготовленного
зарегистрированного RGBD. Единый вход `farm.scene.v1` от сырых COLMAP+PLY ещё ведёт
по прежнему маршруту. Для полного объединения необходимо согласовать сохранённые
пути фотографий, масштаб и вертикаль сцены; простого добавления стадий недостаточно.

Реальные сравнения и исследование:
`output/farm_pipeline/factory/experiments/factory-universal-v12-quality-v1/54_object_scope_quality/`.
