...
Python 3.10+. ML/CUDA work happens inside pinned containers. A small pinned
host virtualenv is supported for plan orchestration and model provisioning; it
does not install Torch or the ML stack.
...
runtimes and requires Python 3.11+; do not add its scientific validators to
the minimal `.venv-control`. The exact commands are in the production runbook
linked above.
