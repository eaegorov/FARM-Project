...
from farm_runtime.source_view_canonicalization import (  # noqa: E402
    canonicalize_source_views,
    require_unique_rescue_sources,
)
from farm_runtime.source_state_canonicalization import (  # noqa: E402
    _canonical_mask_basename,
    remap_source_state,
)
...
def _review_row(state: dict, index: int, object_id: int) -> dict[str, Any]:
    ...
    return {
        ...
        "category": str(
            row_value("object_category", "unresolved object") or "unresolved object"
        ),
        "description": str(row_value("object_caption", "") or ""),
        "attributes": list(row_value("object_key_attributes", []) or []),
        "semantic_tier": str(
            row_value("object_semantic_tier", "geometry_only") or "geometry_only"
        ),
        "semantic_status": str(
...
def _main_impl(transactions: list[Any]) -> int:
    ...
    require_train_fit_refinement(
        ...
        expected_frames_path=rescue_frames_path,
    )

    output_destination = args.output_dir.expanduser().resolve()
    if output_destination.exists():
        raise FileExistsError(
            f"refusing to overwrite combined rescue output: {output_destination}"
        )
    output_destination.parent.mkdir(parents=True, exist_ok=True)
    transaction = tempfile.TemporaryDirectory(
        prefix=f".{output_destination.name}.",
        suffix=".tmp",
        dir=output_destination.parent,
    )
    transactions.append(transaction)
    output = Path(transaction.name)
    ...
    if len(source_images) != len(source_positions) or len(rescue_images) != len(
        rescue_positions
    ):
    ...
    if len(source_frames) != len(source_images) or len(rescue_frames) != len(
        rescue_images
    ):
        raise ValueError(
            "frames manifests and state image tables have different lengths"
        )

    source_frames_input = len(source_frames)
    source_mask_root = args.source_mask_root.expanduser().resolve(strict=True)
    canonicalization = canonicalize_source_views(
        source_frames, source_frames_path, source_positions
    )
    state_remap_audit = remap_source_state(
        state,
        canonicalization,
        source_mask_root,
        staging_mask_root=mask_output,
    )
    source_frames = list(canonicalization.frames)
    source_images = list(state["images"])
    source_positions = list(state["image_positions"])
    require_unique_rescue_sources(
        rescue_frames,
        {str(row["source_image"]) for row in source_frames},
    )

    combined_frames = []
    for row in source_frames:
        item = dict(row)
        item["rgb_path"] = _relative_artifact(
            str(item["rgb_path"]), source_frames_path, output
        )
        item["depth_path"] = _relative_artifact(
            str(item["depth_path"]), source_frames_path, output
        )
    ...
    for rescue_image_id, row in enumerate(rescue_frames):
        item = dict(row)
        item["rgb_path"] = _relative_artifact(
            str(item["rgb_path"]), rescue_frames_path, output
        )
        item["depth_path"] = _relative_artifact(
            str(item["depth_path"]), rescue_frames_path, output
        )
        combined_frames.append(item)
    frames_payload = dict(source_frames_payload)
    frames_payload["frames"] = combined_frames
    frames_payload["full_colmap_rescue"] = {
        "source_frames_input": source_frames_input,
        "source_frames": len(source_frames),
        "source_frames_unique": len(source_frames),
        "source_duplicate_frames_removed": (
            source_frames_input - len(source_frames)
        ),
        "rescue_frames": len(rescue_frames),
        "combined_frames": len(combined_frames),
        "source_view_canonicalization": canonicalization.audit,
    }
    _atomic_json(output / "frames.json", frames_payload)

    for image_id, image in enumerate(source_images):
        item = copy.deepcopy(image)
        if isinstance(item, dict):
            item["image_id"] = image_id
            item["source_ref"] = str(combined_frames[image_id]["rgb_path"])
        source_images[image_id] = item
    state["images"] = source_images
    state["image_positions"] = source_positions

    original_mask_count = 0
    canonical_aliases_created = 0
    canonical_aliases_reused = 0
    for alias in state_remap_audit["mask_aliases"]:
        alias_destination = mask_output / str(alias["canonical_relative"])
        expected_sha = str(alias["sha256"])
        if alias_destination.exists():
            if _sha256(alias_destination) != expected_sha:
                raise ValueError(
                    "canonical mask basename collision has different content: "
                    f"{alias_destination}"
                )
            canonical_aliases_reused += 1
        else:
            _link_file(Path(str(alias["source_path"])), alias_destination)
            original_mask_count += 1
            canonical_aliases_created += 1

    public_state_remap_audit = copy.deepcopy(state_remap_audit)
    for alias in public_state_remap_audit["mask_aliases"]:
        alias.pop("source_path", None)
    source_canonical_contract = {
        "schema": "farm.source-state-canonicalization.v1",
        "source_frames_input": source_frames_input,
        "source_frames_unique": source_image_count,
        "source_image_id_map_sha256": canonicalization.audit[
            "source_image_id_map_sha256"
        ],
        "state_reference_remap": public_state_remap_audit,
    }
    if isinstance(payload, dict) and isinstance(payload.get("state"), dict):
        payload["state"] = state
        metadata = payload.setdefault("meta", {})
        if not isinstance(metadata, dict):
            raise TypeError("source state meta must be a dictionary")
        metadata["source_view_canonicalization"] = source_canonical_contract
    canonical_source_state_path = output / "scene_state_source_canonical.pt"
    canonical_source_temporary = canonical_source_state_path.with_suffix(".pt.tmp")
    torch.save(payload, canonical_source_temporary)
    canonical_source_temporary.replace(canonical_source_state_path)
    ...
    if (
        len(mask_observations) != source_ids.size
        or len(object_image_ids) != source_ids.size
    ):
        raise ValueError(
            "source state mask/image observation tables are not object-aligned"
        )
    refined_mask_count = 0
    refined_mask_aliases: list[dict[str, Any]] = []
    refined_observations_collapsed = 0
    for object_row in accepted_rows:
        ...
        rescue_mask_sha_by_image: dict[int, str] = {}
        for view in object_row.get("views") or []:
            ...
            if not 0 <= rescue_image_id < len(rescue_frames):
                raise ValueError(
                    f"rescue image ID is outside rescue frames: {rescue_image_id}"
                )
            rescue_frame = rescue_frames[rescue_image_id]
            if str(view.get("source_image") or "") != str(
                rescue_frame.get("source_image") or ""
            ):
                raise ValueError("refined rescue view source_image identity differs")
            view_timestamp = view.get("physical_timestamp_ns")
            frame_timestamp = rescue_frame.get(
                "physical_timestamp_ns",
                rescue_frame.get("physical_timestamp"),
            )
            if (
                view_timestamp is not None
                and frame_timestamp is not None
                and int(view_timestamp) != int(frame_timestamp)
            ):
                raise ValueError(
                    "refined rescue view physical timestamp identity differs"
                )
            global_image_id = source_image_count + rescue_image_id
            ...
            source_mask_sha = _sha256(source_mask)
            previous_sha = rescue_mask_sha_by_image.get(global_image_id)
            if previous_sha is not None:
                if previous_sha != source_mask_sha:
                    raise ValueError(
                        "same object/rescue view has conflicting mask artifacts"
                    )
                refined_observations_collapsed += 1
                continue
            canonical_basename = _canonical_mask_basename(
                relative.name, global_image_id
            )
            destination = (
                mask_output
            ...
            if destination.exists():
                if _sha256(destination) != source_mask_sha:
                    raise ValueError(
                        "canonical rescue mask basename collision has different content"
                    )
            else:
                _link_file(source_mask, destination)
            rescue_mask_sha_by_image[global_image_id] = source_mask_sha
            refined_mask_aliases.append(
                {
                    "object_id": object_id,
                    "rescue_image_id": rescue_image_id,
                    "canonical_image_id": global_image_id,
                    "source_relative": str(relative),
                    "source_basename": relative.name,
                    "canonical_relative": (
                        f"object_{object_id:06d}/{canonical_basename}"
                    ),
                    "sha256": source_mask_sha,
                }
            )
            observation = {
                ...
                "image_shape": list(
                    map(int, np.load(destination, allow_pickle=False)["image_shape"])
                ),
                "detection_idx": int(
                    canonical_basename.rsplit("_det_", 1)[1][:-4]
                ),
    ...
    prior_by_id = {
        int(row["id"]): dict(row) for row in prior_catalog if isinstance(row, dict)
    }
    review_queue = []
    for object_id in accepted_ids:
        ...
        row["metric_dimensions_m"] = (
            _array(state["object_box_dimensions_m"])[index].astype(float).tolist()
        )
    ...
    report = {
        ...
        "rescue_frames": len(rescue_frames),
        "combined_frames": len(combined_frames),
        "source_masks_materialized": original_mask_count,
        "canonical_mask_aliases_created": canonical_aliases_created,
        "canonical_mask_aliases_reused": canonical_aliases_reused,
        "assembly_masks_reconstructed": state_remap_audit[
            "assembly_masks_reconstructed"
        ],
        "refined_masks_linked": refined_mask_count,
        "refined_observations_collapsed": refined_observations_collapsed,
        "source_view_canonicalization": canonicalization.audit,
        "source_state_reference_remap": public_state_remap_audit,
        "refined_mask_aliases": refined_mask_aliases,
        ...
        "hashes": {
            ...
            "refinement_sha256": _sha256(refinement_path),
            "source_canonical_state_sha256": _sha256(
                canonical_source_state_path
            ),
    ...
    if output_destination.exists():
        raise FileExistsError(
            f"combined rescue output appeared during build: {output_destination}"
        )
    os.replace(output, output_destination)
...
def main() -> int:
    ...
    try:
        return _main_impl(transactions)
    finally:
