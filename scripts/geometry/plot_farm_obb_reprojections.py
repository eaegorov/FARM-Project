...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Render this exact object ID (repeatable). When supplied, all "
            "requested IDs are rendered and --objects is ignored."
        ),
    )
    parser.add_argument("--views", type=int, default=3)
    parser.add_argument("--dpi", type=int, default=200)
    args = parser.parse_args()
    if not 72 <= int(args.dpi) <= 400:
        raise ValueError("--dpi must lie in [72, 400]")
    ...
    requested_object_ids = list(dict.fromkeys(int(value) for value in args.object_id))
    if any(value < 0 for value in requested_object_ids):
        raise ValueError("--object-id values must be non-negative")
    if requested_object_ids:
        candidates_by_id = {row[2]: row for row in candidates}
        missing = [
            object_id
            for object_id in requested_object_ids
            if object_id not in candidates_by_id
        ]
        if missing:
            raise ValueError(
                "requested objects do not have geometry_pass evidence: "
                + ", ".join(str(value) for value in missing)
            )
        selected = [candidates_by_id[object_id] for object_id in requested_object_ids]
    else:
    ...
    fig = plt.figure(figsize=(19.2, 10.8), dpi=int(args.dpi), facecolor=BG)
    ...
    fig.savefig(args.output, facecolor=BG, dpi=int(args.dpi))
    plt.close(fig)
    print(args.output)
    return 0
