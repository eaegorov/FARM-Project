...
class GeometryGates:
    ...
    min_voxel_component_fraction: float = 0.35
...
def _load_refiner_helpers() -> Any:
    ...
    return importlib.import_module("scripts.geometry.refine_farm_object_geometry")
...
def assess_candidate(
    ...
    *,
    ...
    if float(candidate.get("voxel_component_fraction", 0.0)) < float(
        gates.min_voxel_component_fraction
    ):
        hard_failures.append("voxel_fragmentation")
...
def _baseline_metrics(state: dict, index: int, count: int) -> dict[str, object]:
    ...
    return {
        ...
        "voxel_component_fraction": scalar(
...
def main() -> int:
    ...
    parser.add_argument(
        "--min-voxel-component-fraction", type=float, default=0.35
    )
    ...
    voxel_component_fractions = _state_array(
        state,
    ...
    for index in evaluation_indices:
        ...
        voxel_size_m = float(refiner.VOXEL_BASE_V) * float(1 << voxel_level)
        if index + 1 < voxel_offsets.size and index < voxel_levels.size:
            ...
            if 0 <= start < end <= voxel_flat.size:
                voxel_points = np.asarray(
                    refiner.decode_voxel_keys_numpy(
                        voxel_flat[start:end], voxel_level
                    ),
                    dtype=np.float32,
                ).reshape(-1, 3)
        ...
        if activated:
            ...
            voxel_component_fractions[index] = float(
                candidate["voxel_component_fraction"]
            )
